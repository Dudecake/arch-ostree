...
if [[ ! -z ${boot_file} ]]; then
  ...
  [[ ! -f "/boot/ostree/${os}-${hash:0:64}/initramfs-${kver}.img" ]] && cp "/ostree/deploy/arch/deploy/${hash}/usr/lib/ostree-boot/initramfs-linux.img" "/boot/ostree/arch-${boot_hash}/initramfs-${kver}.img"
